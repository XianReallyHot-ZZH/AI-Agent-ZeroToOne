...
> 它用来回答一个问题：
>
> **为什么一个结构更完整的 agent，不会只靠 `List<Message>` 和一个 `while (true)` 就够了？**
...
## 为什么只靠 `List<Message>` 不够

很多初学者第一次实现 agent 时，会把所有状态都堆进 `List<Message>`。
...
所以它们不该都硬塞进 `List<Message>` 里。

## 关键数据结构

### 1. QueryParams

这是进入 query 引擎时的外部输入。在 Java 中可以用 record 来表达：

```java
/**
 * 查询参数 —— 进入查询引擎时的外部输入，在查询过程中通常不变。
 */
public record QueryParams(
    ...
    Long maxOutputTokensOverride,   // nullable
...
教学版建议你把它显式做成一个 Java record 或可变类：

```java
/**
 * 查询状态 —— 跨迭代真正会变化的控制状态。
 * 使用可变类（非 record），因为状态需要在循环中被修改。
 */
public class QueryState {
    ...
    Long maxOutputTokensOverride = null;
    boolean stopHookActive = false;
    int turnCount = 1;
    TransitionReason transition = null;

    // 从 QueryParams 构造初始状态
    public QueryState(QueryParams params) {
        this.messages = new ArrayList<>(params.messages());
        this.toolUseContext = new HashMap<>(params.toolUseContext());
        this.maxOutputTokensOverride = params.maxOutputTokensOverride();
    }
}
```
...
建议你单独定义一组继续原因，使用 Java 枚举：

```java
/**
 * 转移原因 —— 每次继续下一轮时必须记录"为什么继续"。
 */
public enum TransitionReason {
    TOOL_RESULT_CONTINUATION,    // 工具执行完毕，结果写回
    MAX_TOKENS_RECOVERY,         // 输出被截断，续写
    COMPACT_RETRY,               // 上下文压缩后重试
    TRANSPORT_RETRY,             // 网络/运输层错误，退避重试
    STOP_HOOK_CONTINUATION,      // hook 拦截了结束
    BUDGET_CONTINUATION;         // 预算允许继续推进
}
```
...
```java
public void query(QueryParams params) {
    QueryState state = new QueryState(params);
    // state.messages 从 params 拷贝而来
    // state.turnCount 从 1 开始
    // state.transition 初始为 null
}
```
...
while (true) {
    ...
    TransitionReason transition = state.transition;
    int turnCount = state.turnCount;

    Message response = callModel(messages, ...);
    // ...
}
```
...
- `s01`（`S01AgentLoop.java`）负责建立"最小主循环"
- `s11`（`S11ErrorRecovery.java`）负责建立"错误恢复分支"
...
- 消息层、工具回写、压缩恢复、重试恢复，最终都要回到同一份 `QueryState` 上

更细的 `TransitionReason` 扩展、预算跟踪、prefetch 等扩展，可以放到你把这条最小控制面真正手搓稳定以后再补。
