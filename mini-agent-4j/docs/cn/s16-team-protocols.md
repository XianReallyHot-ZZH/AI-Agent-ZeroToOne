...
启动时确认 `.team/`、`.team/inbox/`、`.team/requests/` 目录被自动创建。

### 案例 1：Shutdown 协议 — 发起关机 + 状态追踪

> 生成一个队友，通过 shutdown_request 发起优雅关机，用 shutdown_response 追踪状态。

先生成一个队友：

```
生成一个叫 alice 的队友（角色 coder），让她检查 src 目录下有多少个 Java 文件
```
...
观察要点：
- Lead 调用 `> shutdown_request:` 工具，传入 teammate=alice
- 返回类似 `Shutdown request a1b2c3d4 sent to 'alice' (status: pending)`
- `.team/requests/` 目录下生成 `a1b2c3d4.json`，内容包含 `{"request_id":"a1b2c3d4","kind":"shutdown","from":"lead","to":"alice","status":"pending","created_at":...,"updated_at":...}`
- alice 在下一轮 drain inbox 时收到 `shutdown_request` 消息，调用 `shutdown_response` 工具（approve=true）
- 请求文件更新：`status` 变为 `"approved"`，新增 `resolved_by`、`resolved_at`、`response` 字段
- alice 设置 `shouldExit=true`，读完最后一轮 inbox 后优雅退出，状态变为 `shutdown`
- 输入 `/team` 确认 alice 状态为 `shutdown`

### 案例 2：Plan Approval 协议 — 队友提交计划 + Lead 审批

> 让队友在执行任务前提交计划，Lead 审批后队友才继续工作。

```
生成一个叫 bob 的队友（角色 coder），让他重构 auth 模块，但要求他先提交计划
```

观察要点：
- bob 被创建后，系统提示词包含 `Submit plans via plan_approval before major work`
- bob 调用 `> plan_approval:` 工具，传入 plan 文本
- 返回 `Plan submitted (request_id=e5f6g7h8). Waiting for lead approval.`
- `.team/requests/e5f6g7h8.json` 被创建，内容包含 `{"kind":"plan_approval","from":"bob","status":"pending","plan":"...","created_at":...}`
- Lead 的 inbox 收到一条 `plan_approval` 类型的消息（含 request_id 和 plan）
- 下一轮 LLM 调用前，inbox 被注入 `<inbox>` 标签
- Lead 调用 `> plan_approval:` 工具审批（approve=true + feedback），返回 `Plan approved for 'bob'`
- 请求文件更新：`status` 变为 `"approved"`，新增 `reviewed_by`、`resolved_at`、`feedback`
- bob 收到 `plan_approval_response` 消息后继续执行

### 案例 3：RequestStore 持久化 + 跨请求追踪

> 验证协议请求落盘到 .team/requests/，理解持久化的意义。

先发起多个请求：

```
生成一个叫 carol 的队友（角色 tester），让她写一个测试文件
```

等 carol 开始工作后：

```
请对 carol 发起关机请求，然后查看请求状态
```

观察要点：
- Lead 调用 `> shutdown_request:` 创建请求，返回 request_id（如 `x1y2z3a4`）
- Lead 调用 `> shutdown_response:` 传入 request_id 查询状态
- 返回 JSON 包含完整记录：`request_id`、`kind`、`from`、`to`、`status`、`created_at`、`updated_at`
- 手动查看 `.team/requests/x1y2z3a4.json`——请求记录以独立 JSON 文件持久化
- 如果 carol 已审批关机，文件中 `status` 为 `"approved"`，`resolved_by` 为 `"carol"`
- 程序重启后，请求文件仍然存在（持久化 vs 内存丢失的区别）

### 案例 4：组合场景 — 多队友 + 多协议并行

> 同时操作两个队友：一个走 plan approval，一个走 shutdown，观察协议互不干扰。

先生成两个队友：

```
生成一个叫 dave 的队友（角色 coder），让他检查 pom.xml 的依赖数量
```

等 dave 开始后：

```
生成一个叫 eve 的队友（角色 reviewer），让她审查 src 目录结构，但先提交计划
```

然后并行操作：

```
请对 dave 发起关机请求，同时审批 eve 提交的计划
```

观察要点：
- 两个协议请求各自生成独立的 request_id 和 `.team/requests/*.json` 文件
- shutdown_request 发到 dave 的 inbox，plan_approval_response 发到 eve 的 inbox
- `/team` 显示 dave 状态变为 `shutdown`，eve 状态为 `working`（继续执行审批通过的计划）
- 两个 `.team/requests/*.json` 文件各自记录了完整的请求生命周期（kind、status、时间戳）
- 协议之间通过 request_id 完全隔离，互不干扰
